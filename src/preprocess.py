...
def get_bar_chart_data(stats):
    ...
    df = stats.loc[
        :, ["price", "sum_influence_3_days", "index_value", "index_variation"]
    ]

    df["sum_influence_3_days"] = df["sum_influence_3_days"] / 1e6

    # Split DF by index variation and range of sum_influence_3_days
    df["variation"] = df["index_variation"].apply(
        lambda x: "Bearish" if x < -0.025 else ("Bullish" if x > 0.025 else "Neutral")
    )

    df["sum_influence_3_days"] = pd.cut(df["sum_influence_3_days"], np.arange(0, 19, 2))

    bar_df = (
        df.groupby(["variation", "sum_influence_3_days"])
        .size()
...
def get_radar_scatter_data(tweets):
    ...
    df["index_range"] = pd.cut(
        df["index_variation"], bins=[0 + i * 0.01 for i in range(101)]
    )
...
def convert_dates(df):
    ...
    """
    df["timestamp"] = pd.to_datetime(df["timestamp"], format="%Y-%m-%d").dt.tz_localize(
        None
    )
    return df
...
def get_timeframe(df):
    ...
    display = (datetime.strptime(start, "%Y-%m-%d") + timedelta(days=30)).strftime(
        "%Y-%m-%d"
    )
