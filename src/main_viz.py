...
def get_tweet(date, query=None):
    ...
    mask_date = (tweets["timestamp"] <= date) & (
        tweets["timestamp"] >= date_minus_3_str
    )
    if query:
        filter_tweets = tweets[tweets["text"].str.contains(query, case=False)]
        tweet = (
            filter_tweets[mask_date]
            .sort_values(by="n_followers")
            .head(5)["text"]
            .str.replace(r"\bhttps?:/\S+", "", regex=True)
            .values
        )
        return tweet
    else:
...
def init_main_figure(df):
    ...
    # generate color list
    colors = [
        "red" if any([i < 0 for i in y_values]) else "green" for y_values in y_pairs
    ]
    ...
    scatter = go.Scatter(
        ...
        hoverinfo="none",
        mode="markers",
        marker=dict(symbol="diamond", size=9, color="black", opacity=0.7),
    )
    ...
    bar_chart.update_traces(
        hovertemplate=hover_template.get_main_vis_bar_chart_hover_template()
    )

    fig.add_trace(bar_chart.data[0], row=2, col=1)

    fig.update_layout(
        coloraxis=bar_chart.layout.coloraxis,
        showlegend=False,
    ...
    )  ### no space between bar, making the graph more visible with large range of x axis
