...
def init_radar_figure(data):
    ...
    # Add buttons
    fig.update_layout(
        title="Hourly visualisation of the index variation <br><sup>{} to {}</sup>".format(
            start, end
        ),
        updatemenus=[
            dict(
        ...
        legend=dict(traceorder="normal", x=0.4),
    )
...
def add_radar_scatter_figure(fig, scatter_data):
    ...
    scatter = go.Scatterpolar(
        r=scatter_data["variation"],
        theta=scatter_data["hour_angle"],
        mode="markers",
        marker=dict(
            color=scatter_data["count"],
            colorscale="Teal",
            colorbar=dict(title="Frequency", x=0.75),
            showscale=True,
        ),
...
def add_radar_trend_figure(fig, mean_df):
    ...
    trend = go.Scatterpolar(
        r=mean_df["index_variation"],
        theta=mean_df["hour_angle"],
        mode="lines",
        connectgaps=True,
